...
void Graph_dump::visit(While_stmt &node) {
    ...
    emit_edge(&node, cond);
    { Graph_dump child{gv_, &node}; cond->accept(child); }

    emit_edge(&node, body);
    { Graph_dump child{gv_, &node}; body->accept(child); }
}

void Graph_dump::visit(If_stmt &node) {
    auto *cond = &node.get_condition();
    auto *then_b = &node.then_branch();
    Statement *else_b = node.contains_else_branch() ? &node.else_branch()
                                                    : nullptr;
    ...
    { Graph_dump child{gv_, &node}; then_b->accept(child); }
...
void Graph_dump::visit(Binary_operator &node) {
    ...
    case Binary_operators::RemDiv:    op_str = "%"; break;
    case Binary_operators::And:       op_str = "&"; break;
    case Binary_operators::Xor:       op_str = "^"; break;
    case Binary_operators::Or:        op_str = "\\|"; break;
    case Binary_operators::LogAnd:    op_str = "\\&&"; break;
    case Binary_operators::LogOr:     op_str = "\\|\\|"; break;
    }
    ...
    emit_edge(&node, l);
    { Graph_dump child{gv_, &node}; l->accept(child); }

    emit_edge(&node, r);
    { Graph_dump child{gv_, &node}; r->accept(child); }
}
...
void Graph_dump::visit(Unary_operator &node) {
    ...
    case Unary_operators::Not:  op_str = "!"; break;
    }
