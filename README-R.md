...
Реализованы `операторы ввода и вывода`. Оператор вывода характеризуется ключевым словом `print` и ожидает после себя выражение, заканчивающееся `;`. Результат выражения выводится в `std::cout`, оператор ввода представляет из себя ключевое слово `?`, он может применяться в любых местах, где подходит использование числа, во время выполнения программы число будет взято из `std::cin`: 
